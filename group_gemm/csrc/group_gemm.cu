...
void initialize(const std::vector<torch::Tensor>& a,
  ...
  for (int i = 0; i < batch_sizes; i++) {
    ...
    // ptr_C_host.at(i) = static_cast<cutlass::float_e4m3_t*>(c[i].data_ptr());
    // ptr_D_host.at(i) = static_cast<cutlass::float_e4m3_t*>(d[i].data_ptr());
  ...
  // cudaMemcpy(host_pointers, ptr_A.get(), n * sizeof(const ElementA*), cudaMemcpyDeviceToHost);
  //   for (int i = 0; i < n; ++i) {
  //       std::cout << "Data at ptr_A[" << k << "] = " << *(host_pointers[i]+k) << std::endl;
  //   }
