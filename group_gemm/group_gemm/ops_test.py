...
import grouped_gemm_backend as gemm
...
def test_m_grouped_gemm_contiguous_list() -> None:
        ...
        # case_list.append(([(128, 256),], [(128, 256),]))
        # case_list.append(([(1280, 1024),], [(1280, 1024),]))
        # case_list.append(([(256, 512),], [(128, 512),]))
        case_list.append(([(128, 256),(128, 512),(1280, 1024)], [(256, 256),(256, 512),(1280, 1024)]))
        print(f"case_list:{case_list}")
        # case_list.append(([(1280, 1901), (1280, 1901), (1280, 1901)], [(2080, 1901), (1819, 1901), (3106, 1901)]))
                # case_list.append(([(1280, 1024),  (1280, 1024)], [(1280, 1024), (1280, 1024)]))
                # case_list.append(([(8192, 8192),  (8192, 8192)], [(8192, 8192), (8192, 8192)]))
        # for num_groups, m, k, n in ((4, 8192, 7168, 4096), (4, 8192, 2048, 7168), (8, 4096, 7168, 4096), (8, 4096, 2048, 7168)):
        # for num_groups, m, k, n in ((4, 8192, 576, 4096), (4, 8192, 2048, 7168), (8, 4096, 7168, 4096), (8, 4096, 2048, 7168)): #((4, 8192, 8192, 8192), (4, 8192, 2048, 2048)):
        case_idx = 0
        for lhs_shape_list,rhs_shape_list in case_list:
            ...
            if case_idx < 10:
                ...
                # # input
                # print(f"x_fp8_inp_list:{x_fp8_inp_list[0]}")
                ...
                print(f' > Performance ({case_idx=}: {t * 1e6:4.0f} us | '
                    f'throughput: {ops / t / 1e12:4.0f} TFLOPS, ')
...
class OpsTest(parameterized.TestCase):
    ...
    def test_m_grouped_gemm_contiguous_list() -> None:
        ...
        # for num_groups, m, k, n in ((4, 8192, 576, 4096), (4, 8192, 2048, 7168), (8, 4096, 7168, 4096), (8, 4096, 2048, 7168)): #((4, 8192, 8192, 8192), (4, 8192, 2048, 2048)):
        case_idx = 0
        for rhs_shape_list,lhs_shape_list in case_list:
            if case_idx < 10:
                # print("case_idx:",case_idx)
                # print("rhs_shape_list:",rhs_shape_list)
                # print("lhs_shape_list:",lhs_shape_list)
                ...
                for i in range(num_groups):
                    # 第i组的m
                    m[i] = rhs_shape_list[i][0]
                    k[i] = rhs_shape_list[i][1]
                    n[i] = lhs_shape_list[i][0]
                    group_info[i] = (m[i],n[i],k[i])
                    x_fp8[i], y_fp8[i], out[i], ref_out[i] = construct(m[i], k[i], n[i])
                    if(m[i] % 128 != 0):
                        m_pad = (m[i] + 127) // 128 * 128
                        x_fp8_pad = torch.empty((m_pad,k[i]),device=x_fp8[i][0].device, dtype=x_fp8[i][0].device)
                ...
                alpha = 0
                beta = 0
                gemm.gmm(x_fp8_inp_list, y_fp8_inp_list, x_fp8_scale_list, y_fp8_scale_list,C_list, out_list,alpha,beta,num_groups,group_info)
