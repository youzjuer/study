...
def optimized_gemm_padding(lhs_temp, rhs_temp, lhs_scales, rhs_scales, out1, 
                          n_0, k_0, num_groups_temp):
    # 统一对齐参数
    n_alignment = 8
    k_alignment = 128
    k_ceil_alignment = 128
    
    # 获取基础维度
    m_temp, k_temp = lhs_temp.shape
    _, n_temp, k_temp_rhs = rhs_temp.shape
    
    # 计算所有需要对齐的维度
    def align_dim(dim, alignment, ceil_alignment=None):
        aligned = (dim + alignment - 1) // alignment * alignment
        if ceil_alignment and aligned % ceil_alignment != 0:
            aligned += alignment
        return aligned
    
    # 一次性计算所有对齐维度
    n_padded = align_dim(n_temp, n_alignment)
    k_padded = align_dim(k_temp, k_alignment, k_ceil_alignment)
    k_padded_rhs = align_dim(k_temp_rhs, k_alignment, k_ceil_alignment)
    
    # 合并内存分配操作
    device = lhs_temp.device
    dtype = lhs_temp.dtype
    ...
    lhs[:, :k_temp] = lhs_temp  # 仅拷贝有效数据 [[7]]

    # 同理处理 rhs 和 rhs_scales
    rhs = torch.empty(
        (num_groups_temp, n_padded, k_padded),
        dtype=rhs_temp.dtype,
        device=rhs_temp.device
    ).fill_(0)
    rhs[:, :n_temp, :k_temp] = rhs_temp


    
    lhs_scales = lhs_scales.T  #(K,M)
    lhs_scale_cols = k_padded // k_alignment
    lhs_scales_pad = torch.nn.functional.pad(
        lhs_scales, (0,0,0, lhs_scale_cols - lhs_scales.size(0)), 
        mode='constant', value=1.0
    ) if lhs_scales.size(0) < lhs_scale_cols else lhs_scales
    lhs_scales_pad = lhs_scales_pad.T
    
    rhs_scale_cols = k_padded_rhs // k_alignment
    rhs_scales_pad = torch.nn.functional.pad(
        rhs_scales, (0, rhs_scale_cols - rhs_scales.size(2)),
        mode='constant', value=1.0
    ) if rhs_scales.size(2) < rhs_scale_cols else rhs_scales
    
    # 输出内存复用优化
    out = out1 if out1.size(1) >= n_padded else torch.empty(
        (m_temp, n_padded), dtype=out1.dtype, device=device
    )
    if out1.size(1) < n_padded:
        out[:, :out1.size(1)] = out1
    
    return lhs, rhs, lhs_scales_pad, rhs_scales_pad, out
def bench_cutlass(
    expected_m_per_group: int,
    n: int,
    k: int,
    ...
    # paddding
    # m_0, k_0 = x_fp8[0].shape
    # if m_0 == 0:
    #     return
    # rhs_num,n_0, _ = y_fp8[0].shape
    # lhs_temp, lhs_scales = x_fp8 
    # rhs_temp, rhs_scales = y_fp8 
    # m_temp, k_temp = lhs_temp.shape
    # rhs_scales_temp = rhs_scales
    # lhs_scales_temp = lhs_scales
    # num_groups_temp, n_temp, k_temp_ = rhs_temp.shape

    # out_temp = out

    # if(k_0 % 128 !=0 or n_0 % 8 != 0):
    #     lhs, rhs, lhs_scales, rhs_scales, c_out = optimized_gemm_padding(
    #             lhs_temp, rhs_temp, lhs_scales, rhs_scales, out1, 
    #             n_0, k_0, num_groups_temp
    #         )
    # else:
    #     lhs, lhs_scales = lhs
...
def main():
    ...
    shape_args = [
        ...
        ShapeArg(expected_m_per_group=128, n=512, k=16384, num_groups=128),
