...
# import fp8_blockwise_scaled_grouped_mm
from torch.utils.benchmark import Timer
from torch.utils.benchmark import Measurement
# Create two normalized matrices and allocate to GPU
# M = K = N = 4096
parser = argparse.ArgumentParser(description='')
parser.add_argument('-M', help='Number of rows, M', default=4096)
parser.add_argument('-N', help='Number of columns, N', default=1024)
parser.add_argument('-K', help='Number of K, K', default=4096)
...
B1 = torch.normal(0,1,size=(args.K, args.N)).to(device=cuda).to(dtype=torch.float)/math.sqrt(args.K)
