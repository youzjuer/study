...
void cutlass_gemm(torch::Tensor & A,  // A matrix (m x k)
                           const torch::Tensor B,  // B matrix (k x n)
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("mm", py::overload_cast<torch::Tensor & ,torch::Tensor,c10::optional<torch::Tensor>>(&cutlass_gemm), py::arg("A"), py::arg("B"), py::arg("out"));
}
